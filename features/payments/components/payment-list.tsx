        ...
        <Card key={payment.id} className="border-0 shadow-soft">
          <CardContent className="flex items-start justify-between gap-3 p-4">
            <div className="flex gap-3">
              ...
              <div>
                <p className="font-semibold text-charcoal-900">
                  {payment.direction === "client_in"
                    ? (payment.clientName ?? "Client payment")
                    : (payment.supplierName ?? "Supplier payment")}
                </p>
            ...
            <div className="flex shrink-0 items-start gap-2">
              <p className="text-sm font-semibold text-charcoal-900">{formatMoney(payment.amount)}</p>
              <DeleteRecordButton id={payment.id} label="payment" action={deletePayment} />
            </div>
          </CardContent>
        </Card>
      ))}
